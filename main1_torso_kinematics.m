...
%% read data
fileID = fopen('1_torso_active.json','r');
mytxt = fscanf(fileID,'%s');
fclose(fileID);
mystruct1 = jsondecode(mytxt);
fldname = fieldnames(mystruct1);

fileID = fopen('2_torso_fixed.json','r');
mytxt = fscanf(fileID,'%s');
fclose(fileID);
mystruct2 = jsondecode(mytxt);

fileID = fopen('3_torso_passive.json','r');
mytxt = fscanf(fileID,'%s');
fclose(fileID);
...
for i = 1:3
    ...
    for j = 1:n_stance2
        frame_temp = frame2(j,1):frame2(j,2);
        data2(:,j,i) = spline(frame_temp, mystruct2.(name)(frame_temp), linspace(frame_temp(1), frame_temp(end), 101));
    end
    for j = 1:n_stance3
